...
class G2PCrop(models.Model):
    ...
    crop_category_id = fields.Many2one('g2p.crop.category', string="Crop Category", compute="_compute_crop_category",
    ...
    land_prep_method_ids = fields.One2many(
        'g2p.crop.land.prep.line',
        'crop_registry_id',
        string="Land Prep Methods"
    )
    ...
    actual_collected_gc = fields.Date(string="Planted Date (GC)")
    actual_collected_ec = fields.Char(string="Planted Date (EC)")
    ...
    seed_planned = fields.Selection([('certified', 'Certified'),
                                     ('basic', 'Basic'),
                                     ('pre-basic', 'Pre-Basic'), ], string="Seed Class")
    ...
    land_category = fields.Selection([('seasonal', 'Seasonal Crops'), ('horticulture', 'Horticulture Crops'), ])
    seasonal_line_ids = fields.One2many(
        "g2p.seasonal.line",
        "crop_registry_id",
        string="Seasonal Crops",
    )
    actual_seasonal_line_ids = fields.One2many(
        "g2p.seasonal.actual.line",
        "crop_registry_id",
        string="Seasonal Crops (Actual)",
    )
    ...
    male_farmers = fields.Integer(string="Required Male Farmers")
    female_farmers = fields.Integer(string="Required Female Farmers")
    total_farmers = fields.Integer(compute="_compute_total_farmers", string="Required Total Farmers")
    ...
    actual_seed_class = fields.Selection([('certified', 'Certified'),
                                          ('basic', 'Basic'),
                                          ('pre-basic', 'Pre-Basic')], string="Seed Class")
    ...
    @api.depends('seasonal_line_ids.crop_name_id', 'seasonal_line_ids.crop_variety_id',
                 'horticulture_line_ids.crop_name_id', 'horticulture_line_ids.crop_variety_id', 'land_category')
    def _compute_primary_crop_details(self):
        for rec in self:
            crop = False
            variety = False
            if rec.land_category == 'seasonal' and rec.seasonal_line_ids:
                crop = rec.seasonal_line_ids[0].crop_name_id.id if rec.seasonal_line_ids[0].crop_name_id else False
                variety = rec.seasonal_line_ids[0].crop_variety_id.id if rec.seasonal_line_ids[0].crop_variety_id else False
            elif rec.land_category == 'horticulture' and rec.horticulture_line_ids:
                crop = rec.horticulture_line_ids[0].crop_name_id.id if rec.horticulture_line_ids[0].crop_name_id else False
                variety = rec.horticulture_line_ids[0].crop_variety_id.id if rec.horticulture_line_ids[0].crop_variety_id else False
            rec.crop_name_id = crop
            rec.crop_variety_id = variety
...
class G2PHorticultureLine(models.Model):
    ...
    _description = "Horticulture Crop Line"
    ...
    start_gc = fields.Date(string="Start GC")
    end_gc = fields.Date(string="End GC")
    start_month = fields.Integer(string="Start Month", store=True, compute="_compute_start_date")
    start_day = fields.Integer(string="Start Day", store=True, compute="_compute_start_date")
    end_month = fields.Integer(string="End Month", store=True, compute="_compute_end_date")
    end_day = fields.Integer(string="End Day", store=True, compute="_compute_end_date")
    ...
    crop_category_id = fields.Many2one(
        ...
        compute="_compute_crop_category",
        store=True,
        readonly=True,
    )
    ...
    seed_planned = fields.Selection([('local', 'Local'), ('improved', 'Improved')], string="Seed Class")
    seed_planned_qty = fields.Float(string="Planned Seed Quantity (kg)")
    seed_planned_fertilizer_type = fields.Selection([
        ('organic', 'Organic'),
        ('inorganic', 'Inorganic'),
    ...
    def _onchange_crop_planned_area(self):
        if self.crop_registry_id and self.crop_planned_area:
            total_planned = sum(self.crop_registry_id.horticulture_line_ids.mapped('crop_planned_area'))
            if total_planned > self.crop_registry_id.land_area:
                attempted_area = self.crop_planned_area
                allocated_area = total_planned - attempted_area
                remaining_area = self.crop_registry_id.land_area - allocated_area
                ...
                return {
                    'warning': {
                        'title': "Area Exceeded",
                        'message': "You entered %.2f ha, but only %.2f ha is remaining out of the total %.2f ha (%.2f ha is already allocated to other crops)." % (attempted_area, remaining_area, self.crop_registry_id.land_area, allocated_area)
                    }
                }
...
class G2PCropLandPrepLine(models.Model):
    ...
    prep_method = fields.Selection([
        ...
        ('manual', 'MANUAL'),
        ('animal', 'ANIMAL'),
        ('combiner_harvester', 'COMBINER HARVESTER'),
        ('planter', 'PLANTER'),
        ('harvester', 'HARVESTER'),
        ('combiner_other', 'COMBINER OTHER'),
...
class G2PHorticultureActualLine(models.Model):
    ...
    collected_gc = fields.Date(string="Planted Date (GC)")
    collected_ec = fields.Char(string="Planted Date (EC)")
    ...
    actual_yield = fields.Float(string="Actual Yield (ha)")
    land_prep_method_ids = fields.One2many("g2p.crop.land.prep.line", "actual_horticulture_line_id", string="Land Prep Methods")
    
    actual_seed_class = fields.Selection([('local', 'Local'), ('improved', 'Improved')], string="Seed Class")
    actual_seed_qty = fields.Float(string="Actual Seed Quantity (kg)")
    actual_fertilizer_type = fields.Selection([
        ('organic', 'Organic'),
        ('inorganic', 'Inorganic'),
    ...
    is_mismatch = fields.Boolean(
        string="Mismatch",
        compute="_compute_is_mismatch",
        store=True,
    )
    
    @api.onchange('actual_crop_area')
    def _onchange_actual_crop_area(self):
        if self.crop_registry_id and self.actual_crop_area:
            total_actual = sum(self.crop_registry_id.actual_horticulture_line_ids.mapped('actual_crop_area'))
            if total_actual > self.crop_registry_id.land_area:
                ...
                return {
                    'warning': {
                        'title': "Area Exceeded",
                        'message': "You entered %.2f ha, but only %.2f ha is remaining out of the total %.2f ha (%.2f ha is already allocated to other actual crops)." % (attempted_area, remaining_area, self.crop_registry_id.land_area, allocated_area)
                    }
                }
    ...
    @api.depends("crop_name_id", "crop_variety_id", "season_id",
        ...
        if self.collected_gc:
            if self.collected_gc > fields.Date.today():
                ...
                return {
                    'warning': {
                        'title': 'Invalid Date',
                        'message': 'Planted Date (GC) cannot be a future date.'
                    }
                }
            cdate = date(
                self.collected_gc.year,
...
class G2PWaterResourceLine(models.Model):
    ...
    method_id = fields.Many2one('g2p.water.source.method', string="Method", domain="[('water_source_id', '=', water_resource_id)]")
...
class G2PSeasonalLine(models.Model):
    ...
    def _onchange_crop_planned_area(self):
        if self.crop_registry_id and self.crop_planned_area:
            total_planned = sum(self.crop_registry_id.seasonal_line_ids.mapped('crop_planned_area'))
            if total_planned > self.crop_registry_id.land_area:
                attempted_area = self.crop_planned_area
                allocated_area = total_planned - attempted_area
                remaining_area = self.crop_registry_id.land_area - allocated_area
...
class G2PSeasonalActualLine(models.Model):
    ...
    actual_yield = fields.Float(string="Actual Yield (ha)")
    land_prep_method_ids = fields.One2many("g2p.crop.land.prep.line", "actual_seasonal_line_id", string="Land Prep Methods")
    ...
    def _onchange_actual_crop_area(self):
        if self.crop_registry_id and self.actual_crop_area:
            total_actual = sum(self.crop_registry_id.actual_seasonal_line_ids.mapped('actual_crop_area'))
            if total_actual > self.crop_registry_id.land_area:
                attempted_area = self.actual_crop_area
                allocated_area = total_actual - attempted_area
                remaining_area = self.crop_registry_id.land_area - allocated_area
    ...
    def _compute_is_mismatch(self):
        for rec in self:
            ...
            matched = False
            for planned in planned_lines:
                if (planned.crop_name_id.id == rec.crop_name_id.id and
                    planned.crop_variety_id.id == rec.crop_variety_id.id and
                    planned.season_id.id == rec.season_id.id):
                    matched = True
                    break
            rec.is_mismatch = not matched
